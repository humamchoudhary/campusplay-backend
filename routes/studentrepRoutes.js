  ...
  try {
    ...
    if (existingRep) {
      return res.status(400).json({
        success: false,
        error: "A rep for this department already exists",
      });
    }
...
router.get("/getNominationForm/:sport", authenticateJWT, async (req, res) => {
  ...
  try {
    ...
    if (nomination) {
      ...
      return res.json({
        success: false,
        message: "No nominations found for this sport",
      });
    }
  } catch (error) {
    ...
    res
      .status(500)
      .json({ success: false, error: "Error fetching nominations" });
  }
});

// Route to submit the nomination form (initial submission)
router.post(
  "/submitNominationForm/:sport",
  authenticateJWT,
  async (req, res) => {
    const { sport } = req.params;
    const {
    ...
    try {
      ...
      res
        .status(500)
        .json({ success: false, error: "Error saving nomination" });
    }
  },
);
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      let nomination = await PlayerNominationForm.findOne({
        sport,
        department: repDepartment,
      });
      ...
      if (!nomination) {
        return res.status(404).json({
          success: false,
          message: "No nomination form found for this sport and department",
        });
      }
      ...
      res
        .status(500)
        .json({ success: false, error: "Error updating nomination" });
    }
  },
);
...
router.get("/getSubmittedForms", authenticateJWT, async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Error fetching submitted forms" });
  }
});
...
router.get("/downloadPDF/:sport", authenticateJWT, async (req, res) => {
  ...
  try {
    ...
    if (!nomination) {
      return res
        .status(404)
        .json({ success: false, message: "No nomination form found" });
    }
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=${sport}_nominations.pdf`,
    );

    doc.text(`Nominations for ${sport} (${department})`, {
      align: "center",
      underline: true,
    });
    doc.moveDown();
...
router.post("/createTrialEvent", authenticateJWT, async (req, res) => {
  ...
  try {
    ...
    const existingEvent = await TrialEvent.findOne({
      department,
      sportCategory,
    });
    if (existingEvent) {
      return res.status(400).json({
        success: false,
        message: "Event already exists for this category in your department.",
      });
    }
