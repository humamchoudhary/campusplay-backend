...
const { SportsCoachUser, SportsRules, TeamRankings, Pools, Schedules } = require('../models');
...
router.post('/create-pools', authenticateJWT, async (req, res) => {
  ...
  try {
    ...
    const rankings = await TeamRankings.find({ category: sport }).sort({ P1: 1 });

    if (!rankings || rankings.length === 0) {
      return res.status(404).json({ success: false, message: 'No teams found for this sport.' });
    }
    ...
    res.json({ success: true, message: 'Pools and schedules created successfully!' });
  } catch (error) {
